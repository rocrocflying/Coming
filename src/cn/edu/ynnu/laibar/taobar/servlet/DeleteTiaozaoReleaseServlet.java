...
public class DeleteTiaozaoReleaseServlet extends HttpServlet {
	@Override
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
	    ...
	    if(flag.equals("detail")){//。。。。。。。。如果用户点击图片查看详情  则此部分处理用户的请求{
			...
			boolean b = true;
			b=pgc.deleteGoodsRelease(goodsid);
			if(b==true){
				//如果为真则跳转到 发布成功页面
				try {
					request.getRequestDispatcher("TiaozaoGoodsSuccess.jsp").forward(request, response);
				} catch (IOException e) {
